...
import { User, BookHeart, BookOpen, Camera, Save, LogOut } from 'lucide-react';
...
export default function ProfilePage() {
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            // Assuming API has an endpoint that accepts type=profile
            const res = await fetch(`${STORAGE_BASE_URL}/api/upload?type=profile`, {
                method: 'POST',
                body: formData
            });
    ...
    const handleSaveProfile = async () => {
        try {
            setLoading(true);
            await api.staff.update(user.staffId, {
                staffName: name,
                phoneNumber: phone,
                address: address,
                profileImage: profileImage
            });
            setMessage('Profile updated successfully! Please re-login to see all changes.');
                    ...
                    {activeTab === 'profile' && (
                        <div className="max-w-xl">
                            ...
                            <div className="flex items-center gap-6 mb-8">
                                <div className="relative w-24 h-24 rounded-full overflow-hidden bg-gray-100 dark:bg-gray-700 border-4 border-white dark:border-gray-800 shadow-md">
                                    {profileImage ? (
                                        <Image src={`${STORAGE_BASE_URL}${profileImage}`} alt="Profile" fill className="object-cover" />
                                    ) : (
